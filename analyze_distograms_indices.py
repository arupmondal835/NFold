...
pdb = md.load_pdb('TEMPLATES/minimized.pdb')
top= pdb.topology
    ...
    for j in range(i+4,L):
        ...
        if value:
            ...
            if sequence[j] is 'G':
                n_j = 'CA'
            #amber numbering
            fo.write("{} {} {}\n".format(int(top.select("resid {} and name {}".format(int(i),n_i)))+1,int(top.select("resid {} and name {}".format(int(j),n_j)))+1,hmin + value*spacing))
            fo.write('\n')
...
#Dihedrals are continous, so there is no been with density above a certain distance
window =50
tight_window = 30
...
for i in range(1,L):
        ...
        if value:
            #amber numbering
            f0.write("{} {} {} {} {}\n".format(int(top.select("resid {} and name {}".format(int(i-1),'C')))+1,int(top.select("resid {} and name {}".format(int(i),'N')))+1,int(top.select("resid {} and name {}".format(int(i),'CA')))+1,int(top.select("resid {} and name {}".format(int(i),'C')))+1,hmin + value*spacing))
            f0.write('\n')
            tight_phi.append(i)
            ax2.plot(disto[:,i])
        else:
...
for i in range(0,L-1):                                                                             
        ...
        if value:
            #amber numbering
            f2.write("{} {} {} {} {}\n".format(int(top.select("resid {} and name {}".format(int(i),'N')))+1,int(top.select("resid {} and name {}".format(int(i),'CA')))+1,int(top.select("resid {} and name {}".format(int(i),'C')))+1,int(top.select("resid {} and name {}".format(int(i+1),'N')))+1,hmin + value*spacing,window))
            f2.write('\n')
            tight_psi.append(i)
            ax2.plot(psi[:,i])
        else:
